...
from infrastructure.db.db_models import BrandDBModel
from domain.models import Brand


class BrandRepositoryDB:
    ...
    def list(self, limit: int = 50, offset: int = 0) -> List[Brand]:
        stmt = select(BrandDBModel).order_by(BrandDBModel.created_at.desc()).limit(limit).offset(offset)
